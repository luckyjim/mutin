...
int32_t rdblock(aadr, sz, buff)
uint32_t aadr,sz,buff[];
{
  int32_t i;
...
int32_t CTsendCMD(int32_t brd,int32_t testmode,int32_t tscope,int32_t writemode,int32_t addr,uint32_t cmd)
{
    int32_t i,j,st,v,sm;
...
int32_t CTsendFake(int32_t brd,int32_t testmode,int32_t tscope,int32_t ttype,int32_t tcnt, char *tdate)
{
    int32_t i,j,st,v,sm;
	st = *(MUTADR(brd,CTSTAT));
	if (TXFREE(st)<12) return (TXFREE(st));
...
void CTsoft_trig(int32_t brd,int32_t tscope)
{
    int32_t i,j;

	printf("Sending a trigger softly to board %i for telescope %i\n",brd,tscope);
...
int32_t CTset_cfg(int32_t brd, int32_t cmd)
{
    int32_t i;
	i= *(MUTADR(brd,CTCFG));
	i|= cmd;
...
int32_t CTunset_cfg(int32_t brd, uint32_t cmd)
{
    int32_t i;
	i= *(MUTADR(brd,CTCFG));
	if (!(i&cmd)) {printf("WARNING: bit(s) not set %08x\n",cmd); return(i);}
	i|=cmd; i-=cmd;
	*(MUTADR(brd,CTCFG))= i;
	i= *(MUTADR(brd,CTCFG));
	return(i);
...
int32_t CTget_reg(int32_t brd, int32_t reg)
{
    int32_t i;
	i= *(MUTADR(brd,reg));
	return(i);
...
CTget_brd_status(char *n,int32_t brd)
{
        char *an;
	int32_t i;
...
char *argv[];
{
  uint32_t lval=0;
  int32_t brd=0;
  int32_t i,j,jj,nbtrig,nn;
  uint32_t	rval,tim,sm,atrig[3],buff32[0x200];
  uint64_t	al,buff64[0x200];
  char *str;
	...
	fd = myFind(0);
	sysadr = (volatile uint32_t *) mmap(NULL,BARSIZE,PROT_WRITE|PROT_READ,MAP_SHARED,fd,0);
	if (sysadr==(uint32_t *)MAP_FAILED) {perror("mmap");exit(-1);}
			...
			for(j=0;j<8;j++)
			  {
			    ...
			    usleep(100);
			    lval= *(MUTADR(brd,CTSTATDA));
			    if ((lval!=0x370000ba)&&(lval!=0x330000b6)&&(lval!=0x310000b4)&&(lval!=0x2f0000b2)) printf("RMSTAT: %08x\n",lval);
			    if (CTsendCMD(i, 1, 44, 0, 0x2000, 0x1234)<12) printf("Error TX FIFO is full\n"); /* Remote read CMD reg */
			    usleep(100);
			    lval= *(MUTADR(brd,CTSTATDA));
			    if (lval!=0xa3bac00d) printf("RMCMD:  %08x\n",lval);
			  }
